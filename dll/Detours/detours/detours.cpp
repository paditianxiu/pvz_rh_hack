...
#ifdef _DEBUG
extern "C" IMAGE_DOS_HEADER __ImageBase;int Detour_AssertExprWithFunctionName(int reportType, const char* filename, int linenumber, const char* FunctionName, const char* msg) {
    int nRet = 0;
...
//
static auto s_pSystemRegionLowerBound = (PVOID)static_cast<ULONG_PTR>(0x70000000);
static auto s_pSystemRegionUpperBound = (PVOID)static_cast<ULONG_PTR>(0x80000000);

//////////////////////////////////////////////////////////////////////////////
//
static auto detour_is_imported(PBYTE pbCode, PBYTE pbAddress) -> bool {
    MEMORY_BASIC_INFORMATION mbi;
    VirtualQuery(pbCode, &mbi, sizeof(mbi));
    __try {
        ...
        if (pbAddress >= ((PBYTE)pDosHeader + pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress) && pbAddress < ((PBYTE)pDosHeader + pNtHeader->OptionalHeader.
            DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress + pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size)) {
            return true;
        }
    }
...
inline auto detour_2gb_below(ULONG_PTR address) -> ULONG_PTR {
    return (address > static_cast<ULONG_PTR>(0x7ff80000)) ? address - 0x7ff80000 : 0x80000;
}

inline auto detour_2gb_above(ULONG_PTR address) -> ULONG_PTR {
...
struct _DETOUR_TRAMPOLINE {
...
};C_ASSERT (sizeof(_DETOUR_TRAMPOLINE)== 72);enum { SIZE_OF_JMP = 5 };inline PBYTE detour_gen_jmp_immediate(PBYTE pbCode, PBYTE pbJmpVal) {
    ...
    *((INT32*&)pbCode)++ = (INT32)((PBYTE)ppbJmpVal);
    ...
    if (pbCode[0] == 0xff && pbCode[1] == 0x25) { // jmp [imm32]
        ...
        PBYTE pbTarget = *(UNALIGNED
        PBYTE *
        )
        &pbCode[2];
        if (detour_is_imported(pbCode, pbTarget)) {
            PBYTE pbNew = *(UNALIGNED
            PBYTE *
            )
            pbTarget;
    ...
    if (pbCode[0] == 0xeb) { // jmp +imm8
        ...
        // Finally, skip over a long jump if it is the target of the patch jump.
            ...
            if (pbCode[0] == 0xff && pbCode[1] == 0x25 && *(UNALIGNED
                INT32 *
            )
            &pbCode[2] == (UNALIGNED
            INT32
    ...
    if (pbCode[0] == 0xe9) { // jmp +imm32
        PBYTE pbNew = pbCode + 5 + *(UNALIGNED
        INT32 *
        )
        &pbCode[1];
    ...
    *ppUpper = (PDETOUR_TRAMPOLINE)hi;
    ...
    if (pbCode[0] == 0x66 && pbCode[1] == 0x0F && pbCode[2] == 0x1F && pbCode[3] == 0x44 && pbCode[4] == 0x00 && pbCode[5] == 0x00) {
        return 6;
    }
    if (pbCode[0] == 0x0F && pbCode[1] == 0x1F && pbCode[2] == 0x80 && pbCode[3] == 0x00 && pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00) {
        return 7;
    }
    if (pbCode[0] == 0x0F && pbCode[1] == 0x1F && pbCode[2] == 0x84 && pbCode[3] == 0x00 && pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00) {
        return 8;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x0F && pbCode[2] == 0x1F && pbCode[3] == 0x84 && pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00 && pbCode[8] == 0x00) {
        return 9;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x66 && pbCode[2] == 0x0F && pbCode[3] == 0x1F && pbCode[4] == 0x84 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00 && pbCode[8] == 0x00 &&
        pbCode[9] == 0x00) {
        return 10;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x66 && pbCode[2] == 0x66 && pbCode[3] == 0x0F && pbCode[4] == 0x1F && pbCode[5] == 0x84 && pbCode[6] == 0x00 && pbCode[7] == 0x00 && pbCode[8] == 0x00 &&
...
inline auto detour_gen_jmp_immediate(PBYTE pbCode, PBYTE pbJmpVal) -> PBYTE {
    PBYTE pbJmpSrc = pbCode + 5;
    *pbCode++ = 0xE9; // jmp +imm32
    *((INT32*&)pbCode)++ = static_cast<INT32>(pbJmpVal - pbJmpSrc);
    return pbCode;
}

inline auto detour_gen_jmp_indirect(PBYTE pbCode, PBYTE* ppbJmpVal) -> PBYTE {
    PBYTE pbJmpSrc = pbCode + 6;
    *pbCode++ = 0xff; // jmp [+imm32]
    *pbCode++ = 0x25;
    *((INT32*&)pbCode)++ = static_cast<INT32>((PBYTE)ppbJmpVal - pbJmpSrc);
    return pbCode;
}

inline auto detour_gen_brk(PBYTE pbCode, PBYTE pbLimit) -> PBYTE {
    while (pbCode < pbLimit) {
        *pbCode++ = 0xcc; // brk;
    }
    return pbCode;
...
inline auto detour_skip_jmp(PBYTE pbCode, PVOID* ppGlobals) -> PBYTE {
    PBYTE pbCodeOriginal;
    ...
    if (pbCode[0] == 0xeb) { // jmp +imm8
        ...
        // Finally, skip over a long jump if it is the target of the patch jump.
            ...
            if (pbCode[0] == 0xff && pbCode[1] == 0x25 && *(UNALIGNED INT32*)&pbCode[2] == 0xFFA) { // jmp [rip+PAGE_SIZE-6]
...
inline auto detour_find_jmp_bounds(PBYTE pbCode, PDETOUR_TRAMPOLINE* ppLower, PDETOUR_TRAMPOLINE* ppUpper) -> void {
    // We have to place trampolines within +/- 2GB of code.
    ULONG_PTR lo = detour_2gb_below((ULONG_PTR)pbCode);
    ULONG_PTR hi = detour_2gb_above((ULONG_PTR)pbCode);
    DETOUR_TRACE(("[%p..%p..%p]\n", (PVOID)lo, pbCode, (PVOID)hi));
...
inline auto detour_does_code_end_function(PBYTE pbCode) -> BOOL {
    if (pbCode[0] == 0xeb || // jmp +imm8
        pbCode[0] == 0xe9 || // jmp +imm32
        pbCode[0] == 0xe0 || // jmp eax
        pbCode[0] == 0xc2 || // ret +imm8
    ...
    if (pbCode[0] == 0xf3 && pbCode[1] == 0xc3) { // rep ret
        return TRUE;
    }
    if (pbCode[0] == 0xff && pbCode[1] == 0x25) { // jmp [+imm32]
        return TRUE;
...
inline auto detour_is_code_filler(PBYTE pbCode) -> ULONG {
    // 1-byte through 11-byte NOPs.
    if (pbCode[0] == 0x90) {
        return 1;
    }
...
};C_ASSERT (sizeof(DETOUR_IA64_BUNDLE)== 16);C_ASSERT (sizeof(_DETOUR_TRAMPOLINE)== 256 + DETOUR_IA64_INSTRUCTIONS_PER_BUNDLE*16);enum { SIZE_OF_JMP = sizeof(DETOUR_IA64_BUNDLE) };inline PBYTE
detour_skip_jmp(PBYTE pPointer, PVOID* ppGlobals) {
    PBYTE pGlobals = NULL;
    PBYTE pbCode = NULL;
    ...
    //                     002024000200100b
    if ((pb[0].wide[0] & 0xfffffc000603ffff) == 0x002024000200100b && pb[0].wide[1] == 0x0004000000203008 && pb[1].wide[0] == 0x001014180420180a && pb[1].wide[1] == 0x07000830c0203008 && pb[2].wide[0]
        == 0x0000000100000010 && pb[2].wide[1] == 0x0080006000000200) {
...
struct _DETOUR_TRAMPOLINE {
    ...
    PBYTE pbRemain; // first instruction after moved code. [free list]
    PBYTE pbDetour; // first instruction of detour function.
};C_ASSERT (sizeof(_DETOUR_TRAMPOLINE)== 104);enum { SIZE_OF_JMP = 8 };inline PBYTE align4(PBYTE pValue) {
    ...
    if ((Opcode & 0xfbf08f00) == 0xf2400c00) { // movw r12,#xxxx
        ...
        if ((Opcode2 & 0xfbf08f00) == 0xf2c00c00) { // movt r12,#xxxx
            ...
            if (Opcode3 == 0xf8dcf000) { // ldr  pc,[r12]
                PBYTE pbTarget = (PBYTE)(((Opcode2 << 12) & 0xf7000000) | ((Opcode2 << 1) & 0x08000000) | ((Opcode2 << 16) & 0x00ff0000) | ((Opcode >> 4) & 0x0000f700) | ((Opcode >> 15) & 0x00000800)
                    | ((Opcode >> 0) & 0x000000ff));
                if (detour_is_imported(pbCode, pbTarget)) {
...
struct _DETOUR_TRAMPOLINE {
    ...
    PBYTE pbRemain; // first instruction after moved code. [free list]
    PBYTE pbDetour; // first instruction of detour function.
};C_ASSERT (sizeof(_DETOUR_TRAMPOLINE)== 184);enum { SIZE_OF_JMP = 12 };inline ULONG fetch_opcode(PBYTE pbCode) {
    return *(ULONG*)pbCode;
...
using PDETOUR_REGION = DETOUR_REGION*;

constexpr ULONG DETOUR_REGION_SIGNATURE = 'Rrtd';
constexpr ULONG DETOUR_REGION_SIZE = 0x10000;
constexpr ULONG DETOUR_TRAMPOLINES_PER_REGION = (DETOUR_REGION_SIZE / sizeof(DETOUR_TRAMPOLINE)) - 1;
static PDETOUR_REGION s_pRegions = nullptr; // List of all regions.
static PDETOUR_REGION s_pRegion = nullptr; // Default region.

static auto detour_writable_trampoline_regions() -> DWORD {
...
static auto detour_runnable_trampoline_regions() -> void {
...
static auto detour_alloc_round_down_to_region(PBYTE pbTry) -> PBYTE {
...
static auto detour_alloc_round_up_to_region(PBYTE pbTry) -> PBYTE {
    // WinXP64 returns free areas that aren't REGION aligned to 32-bit applications.
    ULONG_PTR extra = ((ULONG_PTR)pbTry) & (DETOUR_REGION_SIZE - 1);
    if (extra != 0) {
...
static auto detour_alloc_region_from_lo(PBYTE pbLo, PBYTE pbHi) -> PVOID {
    ...
    for (; pbTry < pbHi;) {
        ...
        if (mbi.State == MEM_FREE && mbi.RegionSize >= DETOUR_REGION_SIZE) {
            ...
            pbTry = detour_alloc_round_up_to_region(static_cast<PBYTE>(mbi.BaseAddress) + mbi.RegionSize);
        }
    }
    return nullptr;
}
...
static auto detour_alloc_region_from_hi(PBYTE pbLo, PBYTE pbHi) -> PVOID {
...
static auto detour_alloc_trampoline_allocate_new(PBYTE pbTarget, PDETOUR_TRAMPOLINE pLo, PDETOUR_TRAMPOLINE pHi) -> PVOID {
    PVOID pbTry = nullptr;
...
static auto detour_alloc_trampoline(PBYTE pbTarget) -> PDETOUR_TRAMPOLINE {
    ...
    if (s_pRegion != nullptr && s_pRegion->pFree != nullptr && s_pRegion->pFree >= pLo && s_pRegion->pFree <= pHi) {
    found_region:
    ...
    pbTarget = pbTarget - static_cast<ULONG>((ULONG_PTR)pbTarget & 0xffff);
...
static auto detour_free_trampoline(PDETOUR_TRAMPOLINE pTrampoline) -> void {
    auto pRegion = (PDETOUR_REGION)((ULONG_PTR)pTrampoline & ~static_cast<ULONG_PTR>(0xffff));
...
static auto detour_is_region_empty(PDETOUR_REGION pRegion) -> BOOL {
    ...
    for (int i = 0; i < DETOUR_TRAMPOLINES_PER_REGION; i++) {
        if (pTrampoline[i].pbRemain != nullptr && (pTrampoline[i].pbRemain < pbRegionBeg || pTrampoline[i].pbRemain >= pbRegionLim)) {
            return FALSE;
        }
    }
...
static auto detour_free_unused_trampoline_regions() -> void {
...
auto WINAPI DetourTransactionBegin() -> LONG {
    ...
    if (InterlockedCompareExchange(&s_nPendingThreadId, static_cast<LONG>(GetCurrentThreadId()), 0) != 0) {
        return ERROR_INVALID_OPERATION;
    }
...
auto WINAPI DetourTransactionAbort() -> LONG {
...
auto WINAPI DetourTransactionCommit() -> LONG {
...
static auto detour_align_from_trampoline(PDETOUR_TRAMPOLINE pTrampoline, BYTE obTrampoline) -> BYTE {
...
static auto detour_align_from_target(PDETOUR_TRAMPOLINE pTrampoline, LONG obTarget) -> LONG {
    for (LONG n = 0; n < ARRAYSIZE(pTrampoline->rAlign); n++) {
...
auto WINAPI DetourTransactionCommitEx(_Out_opt_ PVOID** pppFailedPointer) -> LONG {
    ...
    for (o = s_pPendingOperations; o != nullptr; o = o->pNext) {
        if (o->fIsRemove) {
            ...
            *o->ppbPointer = o->pbTarget;
            ...
            DETOUR_TRACE(("detours: pbTramp =%p, pbRemain=%p, pbDetour=%p, cbRestore=%u\n", o->pTrampoline, o->pTrampoline->pbRemain, o->pTrampoline->pbDetour, o->pTrampoline->cbRestore));

            DETOUR_TRACE(("detours: pbTarget=%p: "
                "%02x %02x %02x %02x " "%02x %02x %02x %02x " "%02x %02x %02x %02x [before]\n", o->pbTarget, o->pbTarget[0], o->pbTarget[1], o->pbTarget[2], o->pbTarget[3], o->pbTarget[4], o->pbTarget
                [5], o->pbTarget[6], o->pbTarget[7], o->pbTarget[8], o->pbTarget[9], o->pbTarget[10], o->pbTarget[11]));

            #ifdef DETOURS_IA64
            ((DETOUR_IA64_BUNDLE*)o->pbTarget)->SetBrl((UINT64)&o->pTrampoline->bAllocFrame); *o->ppbPointer = (PBYTE)&o->pTrampoline->pldTrampoline;
            #endif // DETOURS_IA64
            ...
            *o->ppbPointer = o->pTrampoline->rbCode;
            ...
            #ifdef DETOURS_X86
            PBYTE pbCode = detour_gen_jmp_immediate(o->pbTarget, o->pTrampoline->pbDetour); pbCode = detour_gen_brk(pbCode, o->pTrampoline->pbRemain); *o->ppbPointer = o->pTrampoline->rbCode;
            UNREFERENCED_PARAMETER(pbCode);
            #endif // DETOURS_X86

            #ifdef DETOURS_ARM
            PBYTE pbCode = detour_gen_jmp_immediate(o->pbTarget, NULL, o->pTrampoline->pbDetour); pbCode = detour_gen_brk(pbCode, o->pTrampoline->pbRemain); *o->ppbPointer =
                    DETOURS_PBYTE_TO_PFUNC(o->pTrampoline->rbCode); UNREFERENCED_PARAMETER(pbCode);
            #endif // DETOURS_ARM

            #ifdef DETOURS_ARM64
            PBYTE pbCode = detour_gen_jmp_indirect(o->pbTarget, (ULONG64*)&(o->pTrampoline->pbDetour)); pbCode = detour_gen_brk(pbCode, o->pTrampoline->pbRemain); *o->ppbPointer = o->pTrampoline->
                    rbCode; UNREFERENCED_PARAMETER(pbCode);
            #endif // DETOURS_ARM64

            DETOUR_TRACE(("detours: pbTarget=%p: "
                "%02x %02x %02x %02x " "%02x %02x %02x %02x " "%02x %02x %02x %02x [after]\n", o->pbTarget, o->pbTarget[0], o->pbTarget[1], o->pbTarget[2], o->pbTarget[3], o->pbTarget[4], o->pbTarget[
                    5], o->pbTarget[6], o->pbTarget[7], o->pbTarget[8], o->pbTarget[9], o->pbTarget[10], o->pbTarget[11]));

            DETOUR_TRACE(("detours: pbTramp =%p: "
                "%02x %02x %02x %02x " "%02x %02x %02x %02x " "%02x %02x %02x %02x\n", o->pTrampoline, o->pTrampoline->rbCode[0], o->pTrampoline->rbCode[1], o->pTrampoline->rbCode[2], o->pTrampoline->
                rbCode[3], o->pTrampoline->rbCode[4], o->pTrampoline->rbCode[5], o->pTrampoline->rbCode[6], o->pTrampoline->rbCode[7], o->pTrampoline->rbCode[8], o->pTrampoline->rbCode[9], o->
                pTrampoline->rbCode[10], o->pTrampoline->rbCode[11]));

            #ifdef DETOURS_IA64
            DETOUR_TRACE(("\n")); DETOUR_TRACE(("detours:  &pldTrampoline  =%p\n", &o->pTrampoline->pldTrampoline));
            DETOUR_TRACE(("detours:  &bMovlTargetGp  =%p [%p]\n", &o->pTrampoline->bMovlTargetGp, o->pTrampoline->bMovlTargetGp.GetMovlGp()));
            DETOUR_TRACE(("detours:  &rbCode         =%p [%p]\n", &o->pTrampoline->rbCode, ((DETOUR_IA64_BUNDLE&)o->pTrampoline->rbCode).GetBrlTarget()));
            DETOUR_TRACE(("detours:  &bBrlRemainEip  =%p [%p]\n", &o->pTrampoline->bBrlRemainEip, o->pTrampoline->bBrlRemainEip.GetBrlTarget()));
            DETOUR_TRACE(("detours:  &bMovlDetourGp  =%p [%p]\n", &o->pTrampoline->bMovlDetourGp, o->pTrampoline->bMovlDetourGp.GetMovlGp()));
            DETOUR_TRACE(("detours:  &bBrlDetourEip  =%p [%p]\n", &o->pTrampoline->bCallDetour, o->pTrampoline->bCallDetour.GetBrlTarget()));
            DETOUR_TRACE(("detours:  pldDetour       =%p [%p]\n", o->pTrampoline->ppldDetour->EntryPoint, o->pTrampoline->ppldDetour->GlobalPointer)); DETOUR_TRACE(
                    ("detours:  pldTarget       =%p [%p]\n", o->pTrampoline->ppldTarget->EntryPoint, o->pTrampoline->ppldTarget->GlobalPointer));
    ...
    using DETOURS_EIP_TYPE = ULONG_PTR;
    ...
    for (t = s_pPendingThreads; t != nullptr; t = t->pNext) {
        ...
        if (GetThreadContext(t->hThread, &cxt)) {
            for (o = s_pPendingOperations; o != nullptr; o = o->pNext) {
                if (o->fIsRemove) {
                    if (cxt.DETOURS_EIP >= (ULONG_PTR)o->pTrampoline && cxt.DETOURS_EIP < (ULONG_PTR)o->pTrampoline + sizeof(*o->pTrampoline)) {
                        cxt.DETOURS_EIP = (ULONG_PTR)o->pbTarget + detour_align_from_trampoline(o->pTrampoline, static_cast<BYTE>(cxt.DETOURS_EIP - (ULONG_PTR)o->pTrampoline));

                        SetThreadContext(t->hThread, &cxt);
                    }
                } else {
                    if (cxt.DETOURS_EIP >= (ULONG_PTR)o->pbTarget && cxt.DETOURS_EIP < (ULONG_PTR)o->pbTarget + o->pTrampoline->cbRestore) {
                        cxt.DETOURS_EIP = (ULONG_PTR)o->pTrampoline + detour_align_from_target(o->pTrampoline, static_cast<BYTE>(cxt.DETOURS_EIP - (ULONG_PTR)o->pbTarget));
...
auto WINAPI DetourUpdateThread(_In_ HANDLE hThread) -> LONG {
    ...
    auto t = new NOTHROW DetourThread;
    if (t == nullptr) {
...
auto WINAPI DetourAttachEx(_Inout_ PVOID* ppPointer, _In_ PVOID pDetour, _Out_opt_ PDETOUR_TRAMPOLINE* ppRealTrampoline, _Out_opt_ PVOID* ppRealTarget, _Out_opt_ PVOID* ppRealDetour) -> LONG {
    ...
    auto pbTarget = static_cast<PBYTE>(*ppPointer);
    PDETOUR_TRAMPOLINE pTrampoline = nullptr;
    DetourOperation* o = nullptr;

    #ifdef DETOURS_IA64
    PPLABEL_DESCRIPTOR ppldDetour = (PPLABEL_DESCRIPTOR)pDetour; PPLABEL_DESCRIPTOR ppldTarget = (PPLABEL_DESCRIPTOR)pbTarget; PVOID pDetourGlobals = NULL; PVOID pTargetGlobals = NULL; pDetour = (
        PBYTE)DetourCodeFromPointer(ppldDetour, &pDetourGlobals); pbTarget = (PBYTE)DetourCodeFromPointer(ppldTarget, &pTargetGlobals);
    ...
    if (RtlIsEcCode(reinterpret_cast<DWORD64>(*ppPointer))) {
        DETOUR_TRACE(("*ppPointer is an Arm64EC address (ppPointer=%p). "
            "An Arm64EC address cannot be legitimately detoured with an x64 jmp. " "Mark the target function with __declspec(hybrid_patchable) to make it detour-able. "
    ...
    while (cbTarget < cbJump) {
        ...
        pbSrc = static_cast<PBYTE>(DetourCopyInstruction(pbTrampoline, (PVOID*)&pbPool, pbSrc, nullptr, &lExtra));
    ...
    #ifdef DETOURS_IA64
    pTrampoline->ppldDetour = ppldDetour; pTrampoline->ppldTarget = ppldTarget; pTrampoline->pldTrampoline.EntryPoint = (UINT64)&pTrampoline->bMovlTargetGp; pTrampoline->pldTrampoline.GlobalPointer =
            (UINT64)pDetourGlobals; ((DETOUR_IA64_BUNDLE*)pTrampoline->rbCode)->SetStop(); pTrampoline->bMovlTargetGp.SetMovlGp((UINT64)pTargetGlobals); pTrampoline->bBrlRemainEip.SetBrl(
            (UINT64)pTrampoline->pbRemain);

    // Alloc frame:      alloc r41=ar.pfs,11,0,8,0; mov r40=rp
    pTrampoline->bAllocFrame.wide[0] = 0x00000580164d480c; pTrampoline->bAllocFrame.wide[1] = 0x00c4000500000200;
    // save r36, r37, r38.
    pTrampoline->bSave37to39.wide[0] = 0x031021004e019001; pTrampoline->bSave37to39.wide[1] = 0x8401280600420098;
    // save r34,r35,r36: adds r47=0,r36; adds r46=0,r35; adds r45=0,r34
    pTrampoline->bSave34to36.wide[0] = 0x02e0210048017800; pTrampoline->bSave34to36.wide[1] = 0x84011005a042008c;
    // save gp,r32,r33"  adds r44=0,r33; adds r43=0,r32; adds r42=0,gp ;;
    pTrampoline->bSaveGPto33.wide[0] = 0x02b0210042016001; pTrampoline->bSaveGPto33.wide[1] = 0x8400080540420080;
    // set detour GP.
    pTrampoline->bMovlDetourGp.SetMovlGp((UINT64)pDetourGlobals);
    // call detour:      brl.call.sptk.few rp=detour ;;
    pTrampoline->bCallDetour.wide[0] = 0x0000000100000005; pTrampoline->bCallDetour.wide[1] = 0xd000001000000000; pTrampoline->bCallDetour.SetBrlTarget((UINT64)pDetour);
    // pop frame & gp:   adds gp=0,r42; mov rp=r40,+0;; mov.i ar.pfs=r41
    pTrampoline->bPopFrameGp.wide[0] = 0x4000210054000802; pTrampoline->bPopFrameGp.wide[1] = 0x00aa029000038005;
    // return to caller: br.ret.sptk.many rp ;;
    pTrampoline->bReturn.wide[0] = 0x0000000100000019; pTrampoline->bReturn.wide[1] = 0x0084000880000200; DETOUR_TRACE(("detours: &bMovlTargetGp=%p\n", &pTrampoline->bMovlTargetGp)); DETOUR_TRACE(
            ("detours: &bMovlDetourGp=%p\n", &pTrampoline->bMovlDetourGp));
    #endif // DETOURS_IA64
    ...
    #ifdef DETOURS_X86
    pbTrampoline = detour_gen_jmp_immediate(pbTrampoline, pTrampoline->pbRemain); pbTrampoline = detour_gen_brk(pbTrampoline, pbPool);
    #endif // DETOURS_X86

    #ifdef DETOURS_ARM
    pbTrampoline = detour_gen_jmp_immediate(pbTrampoline, &pbPool, pTrampoline->pbRemain); pbTrampoline = detour_gen_brk(pbTrampoline, pbPool);
    #endif // DETOURS_ARM
    ...
    DETOUR_TRACE(("detours: pbTarget=%p: "
        "%02x %02x %02x %02x " "%02x %02x %02x %02x " "%02x %02x %02x %02x\n", pbTarget, pbTarget[0], pbTarget[1], pbTarget[2], pbTarget[3], pbTarget[4], pbTarget[5], pbTarget[6], pbTarget[7],
        pbTarget[8], pbTarget[9], pbTarget[10], pbTarget[11]));
    DETOUR_TRACE(("detours: pbTramp =%p: "
        "%02x %02x %02x %02x " "%02x %02x %02x %02x " "%02x %02x %02x %02x\n", pTrampoline, pTrampoline->rbCode[0], pTrampoline->rbCode[1], pTrampoline->rbCode[2], pTrampoline->rbCode[3], pTrampoline
        ->rbCode[4], pTrampoline->rbCode[5], pTrampoline->rbCode[6], pTrampoline->rbCode[7], pTrampoline->rbCode[8], pTrampoline->rbCode[9], pTrampoline->rbCode[10], pTrampoline->rbCode[11]));
...
auto WINAPI DetourDetach(_Inout_ PVOID* ppPointer, _In_ PVOID pDetour) -> LONG {
    ...
    auto o = new NOTHROW DetourOperation;
    if (o == nullptr) {
        error = ERROR_NOT_ENOUGH_MEMORY;
    fail:
        s_nPendingError = error;
    ...
    #ifdef DETOURS_IA64
    PPLABEL_DESCRIPTOR ppldTrampo = (PPLABEL_DESCRIPTOR) * ppPointer; PPLABEL_DESCRIPTOR ppldDetour = (PPLABEL_DESCRIPTOR)pDetour; PVOID pDetourGlobals = NULL; PVOID pTrampoGlobals = NULL; pDetour = (
        PBYTE)DetourCodeFromPointer(ppldDetour, &pDetourGlobals); PDETOUR_TRAMPOLINE pTrampoline = (PDETOUR_TRAMPOLINE)DetourCodeFromPointer(ppldTrampo, &pTrampoGlobals);
    ...
    DETOUR_TRACE(("detours:  &rbCode         =%p [%p]\n", &pTrampoline->rbCode, ((DETOUR_IA64_BUNDLE&)pTrampoline->rbCode).GetBrlTarget()));
    DETOUR_TRACE(("detours:  &bBrlRemainEip  =%p [%p]\n", &pTrampoline->bBrlRemainEip, pTrampoline->bBrlRemainEip.GetBrlTarget()));
    ...
    DETOUR_TRACE(("detours:  pldDetour       =%p [%p]\n", pTrampoline->ppldDetour->EntryPoint, pTrampoline->ppldDetour->GlobalPointer)); DETOUR_TRACE(
            ("detours:  pldTarget       =%p [%p]\n", pTrampoline->ppldTarget->EntryPoint, pTrampoline->ppldTarget->GlobalPointer));
    DETOUR_TRACE(("detours:  pbRemain        =%p\n", pTrampoline->pbRemain)); DETOUR_TRACE(("detours:  pbDetour        =%p\n", pTrampoline->pbDetour)); DETOUR_TRACE(("\n"));
    #else // !DETOURS_IA64
    auto pTrampoline = static_cast<PDETOUR_TRAMPOLINE>(DetourCodeFromPointer(*ppPointer, nullptr));
    pDetour = DetourCodeFromPointer(pDetour, nullptr);
    #endif // !DETOURS_IA64
...
static auto DetourPageProtectAdjustExecute(_In_ DWORD dwOldProtect,
                                           _In_ DWORD dwNewProtect) -> DWORD
    //  Copy EXECUTE from dwOldProtect to dwNewProtect.
{
    const bool fOldExecute = ((dwOldProtect & DETOUR_PAGE_EXECUTE_ALL) != 0);
    const bool fNewExecute = ((dwNewProtect & DETOUR_PAGE_EXECUTE_ALL) != 0);
...
_Success_(return != FALSE)auto WINAPI DetourVirtualProtectSameExecuteEx(_In_ HANDLE hProcess,
                                                                        _In_ PVOID pAddress,
                                                                        _In_ SIZE_T nSize,
                                                                        _In_ DWORD dwNewProtect,
                                                                        _Out_ PDWORD pdwOldProtect) -> BOOL
...
auto WINAPI DetourAreSameGuid(_In_ REFGUID left, _In_ REFGUID right) -> BOOL {
    return left.Data1 == right.Data1 && left.Data2 == right.Data2 && left.Data3 == right.Data3 && left.Data4[0] == right.Data4[0] && left.Data4[1] == right.Data4[1] && left.Data4[2] == right.Data4[2]
            && left.Data4[3] == right.Data4[3] && left.Data4[4] == right.Data4[4] && left.Data4[5] == right.Data4[5] && left.Data4[6] == right.Data4[6] && left.Data4[7] == right.Data4[7];
}
